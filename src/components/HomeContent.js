...
export default function HomeContent(props) {
  ...
  const GetAllSmartMetaData = async (tableName) => {
    ...
    HomePageData = smartMetaDataItem.filter(item => item !== null);
  ...
  return (
    <section id="content">
      <div className="content-wrap py-0">
        {/* Safeguard to prevent errors if `homePageData` or `SectionPart` is undefined */}
        {homePageData?.length > 0 && homePageData.map((smartItem, index) => (
